...
import ScrollPageViewBasePage, { PageLifecycle } from './ScrollPageViewBasePage';
  ...
  implements PageLifecycle
{
  ...
  getDataSource(pageCount: number, selectedIndex?: number) {
    ...
    for (let idx = 0; idx < pageCount; idx++) {
      ...
      if (loaded) {
        ...
        this.offsetX = Dimensions.get('window').width * idx;
      }
      dataSource.push({
        index: idx,
        loaded: loaded,
  ...
  select(index: number) {
    ...
    let offsetX = Dimensions.get('window').width * index;
  ...
  onContentSizeChange = (w: number, h: number) => {
    console.log('宽', w, ' ', '高', h);
    if (w > Dimensions.get('window').width && !this.didHandleDefaultSelected) {
  ...
  onScroll = (event: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    if (scrollToRight) {
      idx = Math.floor(offsetX / Dimensions.get('window').width);
    }
  ...
  indexWithOffsetX(offsetX: number): number {
    return Math.ceil(offsetX / Dimensions.get('window').width);
  }
